...
import { AuthorityNotice, CommitBadge, DataTable, EmptyState, ErrorState, LoadingState, MetricCard, PageHeader, ProviderBadge, RunStatusBadge } from "@/components/ui";
...
export default function RunPage() {
  ...
  return <>
    <PageHeader eyebrow="Episode inspection" title={`Seed ${run.seed} · attempt ${run.attempt}`} description={run.instance_id ?? run.run_id} actions={<RunStatusBadge status={run.status} />} />
    <AuthorityNotice />
    <section className={`score-hero verdict-${run.authoritative_verdict ?? "pending"}`}><div><span>Authoritative final score</span><strong>{run.authoritative_reward ?? "—"}</strong><small>{run.authoritative_verdict ?? "pending verifier result"}</small></div><dl><div><dt>Provider / model</dt><dd><ProviderBadge provider={run.provider} /> {run.model}</dd></div><div><dt>Termination</dt><dd>{run.termination_reason ?? "—"}</dd></div><div><dt>Environment</dt><dd><CommitBadge value={run.environment_commit} /></dd></div><div><dt>Record digest</dt><dd><code title={run.record_digest}>{run.record_digest ?? "pending"}</code></dd></div></dl></section>
    ...
    <section className="surface-section"><div className="section-head"><div><span className="eyebrow">Authenticated record</span><h2>Action timeline</h2></div><span className="muted">Capability material removed</span></div><ActionTimeline entries={run.authenticated_timeline ?? []} /></section>
  </>;
}
